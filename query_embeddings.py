...
class EmbeddingsQueryClient:
    ...
    def __init__(self, connection_params: Dict[str, Any], ollama_base_url: str = "http://localhost:11434"):
        """
    ...
    def generate_query_embedding(self, text: str, model: str = "nomic-embed-text") -> Optional[List[float]]:
    ...
    def similarity_search(self, query_text: str, limit: int = 10, model: str = "nomic-embed-text") -> List[Dict[str, Any]]:
...
def print_documents(documents: List[Dict[str, Any]]):
    ...
    for doc in documents:
        filename = doc['filename'][:29] if len(doc['filename']) > 29 else doc['filename']
...
def main():
    ...
    parser.add_argument("--model", default="nomic-embed-text", help="Embedding model for queries")
    ...
    # Initialize client
    client = EmbeddingsQueryClient(db_params, args.ollama_url)
    ...
    try:
        if args.command == 'list':
            ...
            results = client.similarity_search(args.query, args.limit, args.model)
