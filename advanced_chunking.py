...
try:
    ...
    logging.warning("scikit-learn not available. Some advanced chunking methods will be limited.")
...
try:
    ...
    logging.warning("spaCy not available. Named entity recognition features will be limited.")
...
class AdvancedTextChunker:
    ...
    def __init__(self, 
        ...
        self.discourse_patterns = [re.compile(pattern, re.IGNORECASE) for pattern in self.discourse_markers]
        ...
        self.content_patterns = [re.compile(pattern, re.IGNORECASE) for pattern in self.content_markers]
    ...
    def calculate_semantic_boundaries(self, sentences: List[Tuple[str, int, int]]) -> List[ChunkBoundary]:
        """
        ...
        for i in range(1, len(similarities) - 1):
            ...
            if current_sim < threshold and current_sim < prev_sim and current_sim < next_sim:
                sentence_idx = valid_indices[i + 1]  # Boundary after this sentence
                if sentence_idx < len(sentences):
                    position = sentences[sentence_idx][1]  # Start position of next sentence
                    confidence = 1.0 - current_sim  # Lower similarity = higher confidence
    ...
    def calculate_gradient_boundaries(self, text: str, window_size: int = 3) -> List[ChunkBoundary]:
        """
        ...
        if SKLEARN_AVAILABLE:
            ...
            try:
                ...
                for i in range(len(sentences) - window_size):
                    ...
                    window1_vec = np.mean(tfidf_matrix[window1_indices].toarray(), axis=0)
                    window2_vec = np.mean(tfidf_matrix[window2_indices].toarray(), axis=0)
                    ...
                    for i, grad in enumerate(gradients):
                        if grad < threshold:
                            ...
                            if boundary_sentence_idx < len(sentences):
                                ...
                                confidence = (mean_grad - grad) / std_grad if std_grad > 0 else 0.5
    ...
    def semantic_chunk_text(self, text: str) -> List[SemanticChunk]:
        ...
        all_boundaries = semantic_boundaries + gradient_boundaries + structural_boundaries
        ...
        for boundary in all_boundaries:
            if boundary.confidence > 0.3:  # Confidence threshold
                ...
                if not filtered_boundaries or boundary.position - filtered_boundaries[-1].position > min_distance:
        ...
        logger.info(f"Created {len(chunks)} semantic chunks with average size {np.mean([len(c.text) for c in chunks]):.0f} characters")
            ...
            if len(text) > 10000 and self.embedding_generator:
                ...
                if len(boundaries) > 0:
                    ...
                    if gradient_boundaries:
                        chunks = []
                        start = 0
                        for boundary in sorted(gradient_boundaries, key=lambda b: b.position):
                            chunk_text = text[start:boundary.position].strip()
                            if len(chunk_text) >= self.min_chunk_size:
                                chunks.append(chunk_text)
                                start = boundary.position
