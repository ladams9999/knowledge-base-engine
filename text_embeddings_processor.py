    ...
    def _get_advanced_chunker(self):
        ...
        if self._advanced_chunker is None:
            try:
                ...
                logger.warning("Advanced chunking module not available. Using basic chunking.")
                self._advanced_chunker = None
        ...
        if method in ["semantic", "gradient", "adaptive", "hierarchical"]:
            ...
            if advanced_chunker:
                try:
                    ...
                    logger.warning(f"Advanced chunking failed ({method}): {e}. Falling back to basic chunking.")
                    method = "sentences"  # Fallback
            else:
                logger.warning(f"Advanced chunking not available. Falling back to basic method.")
                method = "sentences"  # Fallback
...
class PostgresEmbeddingStorage:
    ...
    def create_tables(self) -> bool:
        ...
        try:
            with self.connection.cursor() as cursor:
                ...
                # Create indexes
                cursor.execute("CREATE INDEX IF NOT EXISTS idx_embeddings_document_id ON text_embeddings(document_id);")
                cursor.execute("CREATE INDEX IF NOT EXISTS idx_embeddings_text_hash ON text_embeddings(text_hash);")
    ...
    def get_or_create_document(self, filepath: str, content_hash: str, metadata: Optional[Dict] = None) -> Optional[str]:
        """
    ...
    def store_embeddings(self, document_id: str, chunks: List[str], embeddings: List[Optional[List[float]]]) -> bool:
        """
        ...
        try:
            with self.connection.cursor() as cursor:
                ...
                cursor.execute("DELETE FROM text_embeddings WHERE document_id = %s", (document_id,))
                ...
                self.connection.commit()
                logger.info(f"Stored {len([e for e in embeddings if e is not None])} embeddings for document {document_id}")
                return True
...
class TextEmbeddingsProcessor:
    ...
    def process_file(self, filepath: str, metadata: Optional[Dict] = None) -> bool:
        ...
        try:
            ...
            document_id = self.storage.get_or_create_document(filepath, content_hash, metadata)
            if not document_id:
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Process text files and generate embeddings")
    parser.add_argument("input_file", help="Path to input text file")
    parser.add_argument("--config", default="config.json", help="Configuration file path")
    parser.add_argument("--chunk-size", type=int, default=1000, help="Chunk size in characters")
    parser.add_argument("--overlap", type=int, default=100, help="Overlap between chunks")
    parser.add_argument("--method", choices=["sentences", "paragraphs", "semantic", "gradient", "adaptive"], 
                       default="sentences", help="Chunking method")
    ...
    parser.add_argument("--db-password", help="Database password (or set POSTGRES_PASSWORD env var)")
    ...
    if not db_password:
        logger.error("Database password must be provided via --db-password or POSTGRES_PASSWORD env var")
        return 1
    ...
    chunker = TextChunker(
        ...
        embedding_generator=embedding_generator  # Pass embedding generator for semantic chunking
    )
